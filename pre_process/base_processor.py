...
class BaseProcessor(abc.ABC):
    ...
    emotions_map: Dict[str, int] = field(default_factory=dict) ####
    ...
    def __post_init__(self):
        ...
        self.create_emotion_map() ####
        self.reverse_speaker = {v: k for k, v in self.speakers_map.items()}
        self.reverse_emotion = {v: k for k, v in self.emotions_map.items()} ####
        self.create_symbols()
    ...
    def create_emotion_map(self):
        """
        Create emotion map for dataset.
        """
        em_id = 0
        for i in self.items:
            emotion_name = i[1].split("|")[-1].split("_")[1]
            if emotion_name not in self.emotions_map:
                self.emotions_map[emotion_name] = em_id
                em_id += 1
    ...
    def get_emotion_id(self, name: str) -> int:
        return self.emotions_map[name]

    def get_emotion_name(self, emotion_id: int) -> str:
        return self.emotions_map[emotion_id]
    ...
    def _load_mapper(self, loaded_path: str = None):
        ...
        self.emotions_map = data["emotions_map"]
        ...
        for key in all_data_keys:
            if key not in ["speakers_map","emotions_map","symbol_to_id", "id_to_symbol"]:
    ...
    def _save_mapper(self, saved_path: str = None, extra_attrs_to_save: dict = None):
        ...
        with open(saved_path, "w") as f:
            full_mapper = {
                ...
                "emotions_map":self.emotions_map,
