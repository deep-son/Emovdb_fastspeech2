...
class TFFastSpeechVariantPredictor(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        if config.n_speakers > 1:
            ...
            self.decoder_emotion_embeddings = tf.keras.layers.Embedding(
                config.n_emotions,
                config.encoder_self_attention_params.hidden_size,
                embeddings_initializer=get_initializer(config.initializer_range),
    ...
    def call(self, inputs, training=False):
        """Call logic."""
        encoder_hidden_states, speaker_ids, emotion_ids, attention_mask = inputs
        ...
        emotion_features = tf.math.softplus(self.speaker_fc(emotion_embeddings))
        ...
        encoder_hidden_states += (extended_speaker_features+extended_emotion_features)
...
class TFFastSpeech2(TFFastSpeech):
    ...
    def call(
        ...
        **kwargs,
        ...
        duration_outputs = self.duration_predictor(
            [last_encoder_hidden_states, speaker_ids,emotion_ids, attention_mask]
        )  # [batch_size, length]

        f0_outputs = self.f0_predictor(
            [last_encoder_hidden_states, speaker_ids,emotion_ids, attention_mask], training=training
        )
        ...
        decoder_output = self.decoder(
            [length_regulator_outputs, speaker_ids,emotion_ids, encoder_masks, masked_decoder_pos],
            training=training,
        )
    ...
    def _inference(
        self, input_ids, speaker_ids, emotion_ids, speed_ratios, f0_ratios, energy_ratios, **kwargs,
    ):
        """Call logic."""
        attention_mask = tf.math.not_equal(input_ids, 0)
        ...
        f0_outputs = self.f0_predictor(
            [last_encoder_hidden_states, speaker_ids,emotion_ids, attention_mask], training=False
        )
